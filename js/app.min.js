(() => {
  ...
  let i = (t, e = !1, o = 500, n = 0) => {
    ...
    if (s) {
      ...
      if (
        ...
        new SmoothScroll().animateScroll(s, "", r);
      else {
      ...
      (this.оbjects = []),
        ...
        this.mediaQueries.forEach((t) => {
          const e = t.split(","),
            ...
            n = e[1],
            s = this.оbjects.filter(({ breakpoint: t }) => t === n);
  ...
  const d = document.querySelector(".about__video-box"),
    u = document.getElementById("videoAbout");
  (document.getElementById("startVideo").onclick = () => {
    d.classList.contains("play")
      ? (u.pause(), d.classList.remove("play"), (u.volume = 0.8))
      : (u.play(), d.classList.add("play"), (u.volume = 0.8));
  }),
    (d.onclick = () => {
      d.classList.contains("play")
        ? (u.pause(), d.classList.remove("play"), (u.volume = 0.8))
  ...
  document.querySelectorAll(".popup__button").forEach((t) => {
    t.onclick = () => {
      t.closest(".popup").classList.add("hidden");
    };
  });
  const h = document.querySelectorAll(".error-how__content-button"),
    m = document.querySelectorAll(".error-how"),
    g = document.querySelector(".how__errors"),
    p = g.offsetWidth - g.firstElementChild.offsetWidth,
    f = g.offsetHeight - g.firstElementChild.offsetHeight;
  m.forEach((t) => {
    (t.style.left = `${0 + Math.floor(Math.random() * p - 1)}px`),
      (t.style.top = `${0 + Math.floor(Math.random() * f - 1)}px`);
  }),
    h.forEach((t) => {
      t.onclick = () => {
        t.closest(".error-how").classList.add("hidden");
      };
    });
  const y = document.querySelectorAll(".draggable");
  let w = 0,
    L = 0,
    v = null,
    E = !1,
    b = 2;
  function S(t) {
    (v = t.target.parentElement),
      (v.style.zIndex = b),
      b++,
      (E = !0),
      (w = t.clientX - v.getBoundingClientRect().left),
      (L = t.clientY - v.getBoundingClientRect().top);
  }
  y.forEach((t) => {
    t.parentElement.onclick = () => {
      (t.parentElement.style.zIndex = b), b++;
    };
  }),
    y.forEach((t) => {
    ...
    document.addEventListener("mousemove", function (t) {
      if (E && v) {
        const e =
            t.clientX -
            w -
            document.querySelector(".how__content").getBoundingClientRect()
              .left,
          o =
            t.clientY -
            L -
            document.querySelector(".how__content").getBoundingClientRect().top;
        (v.style.left = `${e}px`), (v.style.top = `${o}px`);
      }
    }),
    document.addEventListener("mouseup", function () {
      (E = !1), (v = null);
    });
  document.querySelector(".tokenomics__bottom-button").onclick = () => {
    document.querySelector(".tokenomics").classList.add("tok-active"),
      setTimeout(() => {
        document.querySelector(".tokenomics__ca").style.display = "flex";
      }, 150);
  };
  const k = document.querySelector(".bg-fixed__image img");
  let _ = 1;
  setInterval(() => {
    k.setAttribute("src", `img/left/0${_}.gif`), _ < 8 ? _++ : (_ = 1);
  }, 2e3);
  const q = document.querySelector(".preloader__animation img"),
    x = document.querySelectorAll(".preloader__btn"),
    W = document.getElementById("prelBtnYes"),
    $ = document.getElementById("prelBtnNo");
  document.documentElement.classList.add("lock"),
    (W.onclick = () => {
      q.setAttribute("src", "img/preloader/exit.gif"),
        setTimeout(() => {
    ...
    ($.onclick = () => {
      q.setAttribute("src", "img/preloader/no.gif");
    }),
    (window.onload = () => {
      setTimeout(() => {
